...
## How to use `tidyverse` {.scrollable}
...
- R has a *functional style*, which means the structure is typically: `new_data <- function(data, extra_parameters)`
  - `function` describes your **action**, what you want to do
  - `data` is the data that you are execute the action on
  - `extra_parameters` are (optional) **settings** that changes how the action is performed
...
- Pipes transfer the data from its left-hand side (LHS) to the function on its right-hand side (RHS) **as the first parameter of that function**
- The structure: `new_data <- data %>% function(extra_parameters)`
...
::: {.callout-note .fragment}
Every function under `tidyverse` will take the data as the first parameter, so everything can be piped!
:::
...
```{r}
# read in data
covid_cases <- read_rds(here::here("data/covid_cases.rds"))
covid_cases
```
...
:::{.incremental}
- Instead of going through each element and apply an action, we can an action to all elements at once
...
- R has a **functional** style, which means vectorisation fits better, and is more intuitive and elegant
...
- Vectorisation shines when there are more complex actions, where you have to write your own functions
